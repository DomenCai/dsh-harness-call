...
export const codexAdapter = {
    ...
    build(req) {
        if (req.mode === 'resume') {
            ...
            if (req.effort !== undefined)
                argv.splice(3, 0, '-c', `model_reasoning_effort="${req.effort}"`);
        ...
        if (req.access === 'full-access')
            argv.push('--dangerously-bypass-approvals-and-sandbox');
        if (req.effort !== undefined)
            argv.splice(2, 0, '-c', `model_reasoning_effort="${req.effort}"`);
        argv.push('-');
        return { argv, stdin: req.prompt, env: {} };
    },
    ...
    finalize(state, outcome, info) {
        const errors = [];
        if (info.timedOut)
            errors.push(`codex exceeded ${info.timeoutSeconds}s and was terminated`);
        const exit = exitFailure('codex', outcome);
        if (exit !== undefined)
            errors.push(exit);
        if (state.sessionId === undefined)
            errors.push('missing codex thread.started event');
        ...
        if (state.completed !== true)
            errors.push('missing codex turn.completed event');
        if (state.text === '')
            errors.push('missing codex agent message');
        return {
            ok: errors.length === 0,
            text: state.text,
...
function fileChanges(item) {
    ...
    for (const entry of changes) {
        ...
        if (path === undefined || change === undefined)
            continue;
