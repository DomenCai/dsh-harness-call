...
const NO_CHANNEL = { ready: false, error: undefined };
/**
 ...
 */
export function createRunFeed(resolve) {
    ...
    const setChannel = (next) => {
        if (channel.ready === next.ready && channel.error === next.error)
            return;
        channel = next;
        notify();
    };
 ...
 */
export function useRoster(feed, active) {
    const subscribe = useCallback((listener) => (active ? feed.subscribe(listener) : () => { }), [feed, active]);
 ...
 */
export function useChannel(feed, active) {
    const subscribe = useCallback((listener) => (active ? feed.subscribe(listener) : () => { }), [feed, active]);
 ...
 */
export function matchRun(runs, callId, harness) {
 ...
 */
export function useRunDetail(feed, runId, settled) {
    ...
    const load = useCallback(async () => {
        ...
        if (issued !== generation.current || detail === undefined)
            return;
        if (detail === 'unknown') {
            if (settled)
                setTerminal(true);
        ...
        if (detail.events.length > 0)
            events.current = [...events.current, ...detail.events];
        if (detail.summary.phase === 'done')
            setTerminal(true);
 ...
 */
export function readResult(block) {
    if (!('kind' in block))
        return undefined;
    let parsed;
