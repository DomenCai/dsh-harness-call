...
import { defaultHarnessCallSettings } from "../shared/policy.js";
import { HARNESS_CALL_CONTRIBUTION, SERVICE_KEY } from "../shared/wire.js";
...
import { HarnessSettingsSection } from "./SettingsSection.js";
...
export function apply(ctx) {
    ...
    const loadSettings = async () => {
        ...
        try {
            ...
            if (api !== remote || generation !== settingsGeneration)
                return;
            if (!result.ok) {
    ...
    const closePanel = () => { publish(undefined); };
    ...
    function OverlayEntry(props) {
        ...
        if (shown === undefined || foreign)
            return null;
        return _jsx(HarnessPanel, { target: shown, feed: feed, t: t, onClose: closePanel }, shown.callId);
    }
    ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: OVERLAY_ID, order: OVERLAY_ORDER }, OverlayEntry));
    ctx.slots.inject('tool.call.toolview', () => ctx.slots.register({ name: 'tool.call.toolview', key: TOOL_NAME }, props => (_jsx(HarnessCallCard, { callId: props.callId, sessionId: props.sessionId, block: props.block, feed: feed, t: t, onOpen: publish }))));
