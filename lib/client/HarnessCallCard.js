...
import { brief, matchRun, readArgs, readResult, seconds, useRoster } from "./runs.js";
...
export function HarnessCallCard(props) {
    ...
    return (_jsxs("div", { className: css.card, title: t('card.openRunning'), onClick: open, children: [_jsxs("div", { className: css.head, children: [_jsx(StateDot, { state: "ongoing", className: css.dot }), _jsx("span", { className: css.label, children: `${label} · ${t('card.running')}` })] }), _jsx("div", { className: css.status, children: summary === undefined || summary.phase === 'starting'
                    ? t('card.starting')
                    : liveStatus(summary, t) }), liveSession !== null && liveSession !== undefined && (_jsx("div", { className: css.meta, children: `session ${liveSession}` })), args.prompt !== undefined && (_jsx("div", { className: css.promptExcerpt, children: brief(args.prompt, PROMPT_EXCERPT_CHARACTERS) }))] }));
}
